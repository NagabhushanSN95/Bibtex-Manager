...
def export_bib(configs_filepath: Path):
    ...
    # Template
    if 'template' in configs.keys():
        template_filepath = Path(f'../res/templates/{configs["template"]}.bib')
        if template_filepath.exists():
            with open(template_filepath.as_posix(), 'r') as template_file:
                export_strings.append(''.join(template_file.readlines()))
        else:
            print(f'{template_filepath.as_posix()} does not exist')
...
def demo1():
    configs_filepath = Path('../res/configs/CVPR.json')
